// based on official standard net/mail package.
//
...
func lookup(charsetName string) (e encoding.Encoding) {
	defer func() {
		recover()
		return
	}()
...
var LookupCharsetConverter = func(r io.Reader, charset string) (rr io.Reader, err error) {
	e := lookup(charset)
	if e == nil {
		err = errors.New("mail: could not find charset converter")
		return
	}

	rr = transform.NewReader(r, e.NewDecoder())
	return
}

func TransferEncodingDecoder(r io.Reader, transferEncoding string) (io.Reader, error) {
	switch transferEncoding {
	case "base64":
		return base64.NewDecoder(base64.StdEncoding, r), nil
	case "quoted-printable":
		return newQuotedPrintableReader(r), nil
	case "7bit", "8bit", "binary":
		return r, nil
	case "":
		// decoded or need encoding detection...?
		return r, nil
	}
	return nil, fmt.Errorf("mail: Unsupported Content-Transfer-Encoding type: %v", transferEncoding)
}
...
func (h Header) GetWithRFC2047Decoded(key string) string {
	return DecodeRFC2047(h.Get(key))
}
...
func DecodeRFC2047(s string) string {
	...
	cont := false
	for {
		begin, end := strings.Index(s, "=?"), strings.Index(s, "?=")
		if begin < 0 || end < 0 {
			break
		}
		dec, err := decodeRFC2047Word(s[begin : end+2])
		if err != nil {
			buf.WriteString(s[:end+2])
			s = s[end+2:]
			cont = false
			continue
		}
		if cont && !isSpaceOnly(s[:begin]) {
...
func decodeRFC2047Word(s string) (string, error) {
	...
	if len(fields) != 5 || fields[0] != "=" || fields[4] != "=" {
		return "", errors.New("mail: invalid RFC 2047 encoding")
	}
	...
	dec, err := LookupCharsetConverter(r, charset)
	if err != nil {
		return "", fmt.Errorf("mail: could not convert charset: %q", charset)
	}
